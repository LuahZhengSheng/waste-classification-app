...
class StaffRecycleActivityController extends GetxController {
  static StaffRecycleActivityController get instance => Get.find();
  ...
  /// Load activities processed by current staff
  void _loadActivities() {
    try {
      isLoading.value = true;
      final staffId = _authRepo.authUser?.uid;
      if (staffId == null) {
        FLoaders.errorSnackBar(title: 'Error', message: 'Staff not authenticated');
        return;
      }

      // Get all completed activities where centerStaffId matches current staff
      _activityRepo.getStaffActivitiesStream(staffId).listen((allActivities) {
        // Filter activities processed by this staff
        final staffActivities = allActivities
            .where((activity) => activity.centerStaffId == staffId && activity.isCompleted)
            .toList();

        activities.assignAll(staffActivities);
        applyFilters();
  ...
  /// Get total points processed by staff
  int get totalPointsProcessed {
    return activities.fold(0, (sum, activity) => sum + activity.pointsEarned);
  }

  /// Get total weight processed by staff
  double get totalWeightProcessed {
    return activities.fold(0.0, (sum, activity) => sum + activity.weight);
  }
